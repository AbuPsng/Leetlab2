...
export const executeCode = async (req, res) => {
    ...
    try {
        ...
        const submissionObject = stdin.map(input => ({
            source_code, language_id, stdin: input,
        }))

        // 3. Send batch of submission to judge0
        const submitResponse = await submitBatch(submissionObject)
        ...
        // 7. If all passed is true ..then mark the problem as solver for the current user
